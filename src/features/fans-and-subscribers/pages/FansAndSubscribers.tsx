...
import { FansTab } from '../components';
...
export const FansAndSubscribers: React.FC = () => {
    ...
    const getContentTabs = () => {
        ...
        if (isCreator) {
            return [{ id: 'video', label: 'Video', icon: FiVideo }];
        }
        return [
    ...
    return (
        <VStack align="stretch" gap={6} px={7}>
            ...
            {/* Tab Content — the tab key forces a clean remount per vertical */}
            <FansTab key={activeTab} mediaType={activeTab === 'video' ? 'video' : 'music'} />
        </VStack>
    );
};
