...
import {
    AD_DISPUTE_SUBJECT_TYPE_OPTIONS,
...
/**
 * Ad disputes — REUSES the Group 5 dispute case workflow, scoped to the
 * advertising subject types (AdCampaign / AdWallet). The backend filters on a
 * single `subjectType`; "All ad subjects" sends no subject filter.
 */
const AdDisputesPage: React.FC = () => {
    const canView = useHasPermission('DisputesView');
    const canManage = useHasPermission('DisputesManage');
    const [query, setQuery] = React.useState<DisputeQuery>({
        page: 1,
        pageSize: PAGE_SIZE,
    ...
    return (
        <AdminPageLayout
            title="Ad Disputes"
            subtitle="Refund and complaint cases against ad campaigns and advertiser wallets."
            breadcrumbs={[{ label: 'Advertising' }, { label: 'Disputes' }]}
            actions={
                canView && canManage ? (
                    <Button
                        size="sm"
        ...
        >
            {!canView ? (
                ...
                <>
                        ...
                        filters={[
                            {
                                ...
                                options: AD_DISPUTE_SUBJECT_TYPE_OPTIONS,
                                width: '170px',
                            },
                            {
                                key: 'status',
                    ...
                    {error ? (
                        <AdminError error={error} message="Could not load ad disputes." />
                    ) : (
                        <DataTable
                            columns={columns}
                            rows={data?.items ?? []}
                            ...
                            emptyTitle="No cases"
                            emptyDescription="No advertising disputes match the current filters."
                            pagination={
                                data
                                    ? {
                                          page: data.page,
...
const DisputeDrawer: React.FC<{
    ...
    const submitForm = () => {
        ...
        if (form === 'assign')
            assign.mutate(
                {
                    id: disputeId,
                    payload: { assigneeUserId: assigneeUserId.trim(), note: note.trim() || null },
                },
                { onSuccess: done },
            );
        else if (form === 'status')
    ...
    const formPending =
        assign.isPending || setStatus.isPending || addNote.isPending || resolve.isPending;
...
const OpenCaseDialog: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    const [subjectType, setSubjectType] = React.useState('AdCampaign');
    ...
    const submit = () => {
        if (!valid) {
            toast.error(
                'Missing fields',
                'A raised-by user and a 10+ character description are required.',
            );
            return;
        }
                ...
                <Dialog.Positioner>
                    <Dialog.Content maxW="520px" p={6} borderRadius="20px" position="relative">
                        ...
                        <Text
                            ...
                            mb={4}
                        >
                            Open an ad dispute case
                        </Text>
                        <VStack align="stretch" gap={3}>
                            <SimpleGrid columns={2} gap={3}>
                                <DialogField label="Subject type">
                                    <Select
                                        options={AD_DISPUTE_SUBJECT_TYPE_OPTIONS.filter(
                                            (o) => o.value !== 'All',
                                        )}
                                        value={subjectType}
                            ...
                            <DialogField label="Subject ID (optional)">
                                <Input
                                    ...
                                    size="sm"
                                    placeholder="Campaign / wallet GUID"
                                />
                            </DialogField>
                            <SimpleGrid columns={2} gap={3}>
