...
export const VideoAdsTab: React.FC = () => {
    ...
    const videoCampaigns = getCampaignsByType('video');

    // Filter campaigns based on search query
    const filteredCampaigns = videoCampaigns.filter((campaign) =>
        campaign.title.toLowerCase().includes(searchQuery.toLowerCase())
    );

    const handleEdit = (campaignId: string) => {
        navigate(`/ads/create-campaign?tab=video&id=${campaignId}`);
    };
            ...
            <Box bg="white" borderRadius="lg" p={8} textAlign="center">
                <VStack gap={4}>
                    ...
                    <Text fontSize="sm" color="gray.500">
                        Create your first video ad campaign to get started
                    </Text>
                </VStack>
            </Box>
        );
